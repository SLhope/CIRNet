...
class ShadowDataset(torch.utils.data.Dataset):
    def __init__(self, opt, datadir, dirA='shadow', dirB='noshadow',imgsize=None):
        super(ShadowDataset, self).__init__()
        self.opt = opt
    ...
    def __getitem__(self, index):
        ...
        data = {'input': M,  'target_t': T, 'fn': fnA[:-4]}
        return data
...
class TestDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        m_img = cv2.cvtColor(m_img,cv2.COLOR_BGR2RGB)
        M = np.transpose(np.float32(m_img)/255.0,(2,0,1))
        data = {'input': M, 'target_t': torch.zeros([1,0]), 'fn': fn[:-4]}
        return data
