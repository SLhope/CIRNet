...
class RefinedNet(nn.Module):
    def __init__(self,in_channels=3,out_channels=3,enhance='de',ppm=True,bilinear=True):
        ...
        self.dilation_block = nn.Sequential(
            DilationBlock(d_channels[2], d_channels[2], d_channels[2], dilations=[2], gatedconv=False),
            DilationBlock(d_channels[2], d_channels[2], d_channels[2], dilations=[4], gatedconv=False),
            DilationBlock(d_channels[2], d_channels[2], d_channels[2], dilations=[8], gatedconv=False),
            SELayer(d_channels[2] * 3)  
        )
        ...
        # first decoder
        self.up1 = UpBlock(u_channels[0]*3,u_channels[1],u_channels[1],add_channels=d_channels[1],kernels=[5,5],gatedconv=False,bilinear=bilinear)
        # second decoder
    ...
    def forward(self,x,iters=1):
        ...
        refined_features = self.dilation_block(down2)
        up1 = self.up1(refined_features, down1)

        up2 = self.up2(up1,preconv)
        y = self.coarse(torch.cat([x,up2],dim=1))
        return {'refined':y}
...
class DiscFeat(nn.Module):
    ...
    def forward(self,x,require_feat=False): 
        feat = self.feat(x)
        y = self.disc(feat)
